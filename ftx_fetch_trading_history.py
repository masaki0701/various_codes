...
traded_symbol_list = pd.read_pickle("traded_symbol_list.pkl")
traded_symbol = traded_symbol_list[0]
analysis_time = pd.read_pickle("analysis_time.pkl")
...
while end_time < time.time():
    ...
    if progress_rate > milestone_percentage_unit*cnt:
        ...
        cnt += 1
    time.sleep(freq)
print("now accululated data to present time for {}".format(traded_symbol))
# you get historical trades for analysis time to present time
hist_trades = pd.DataFrame(hist_trades)[pd.DataFrame(hist_trades)["time"].apply(lambda x : x[:19]).apply(lambda x : datetime.strptime(x, '%Y-%m-%dT%H:%M:%S')).apply(lambda x : x.timestamp()) >= time.time() - analysis_time]
...
while True:
    try:
        ...
        print("1st executions saved for {}".format(traded_symbol))
...
while True:
    while True:
        try:
            ...
            try:
                tmp = pd.DataFrame(trades)["time"].apply(lambda x : x[:19]).apply(lambda x : datetime.strptime(x, '%Y-%m-%dT%H:%M:%S'))
            except Exception as e:
                print("error in row number : {}".format(location()[2]))
                print(e)
    ...
    hist_trades = hist_trades[hist_trades["time"].apply(lambda x : x[:19]).apply(lambda x : datetime.strptime(x, '%Y-%m-%dT%H:%M:%S')).apply(lambda x : x.timestamp()) >= time.time() - analysis_time]
    hist_trades = pd.DataFrame(hist_trades)[pd.DataFrame(hist_trades)["id"].duplicated() == False].reset_index(drop = True)
    while True:
        try:
            pd.to_pickle(len(hist_trades), "{}_len_of_data.pkl".format(traded_symbol))
            hist_trades.to_csv("{}_executions.csv".format(traded_symbol))
            print(len(hist_trades))
            print("data refreshed for {}".format(traded_symbol))
            ...
            if saved == False and datetime.today().hour == 0:
                ...
                # print("data saved for {}".format(traded_symbol))
                saved = True
